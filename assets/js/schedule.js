...
function renderSchedules(data) {
  ...
  data.forEach((item, index) => {
    ...
    const passedLabel = item.isPassed
      ? `<span class="ml-2 text-xs text-gray-400">${getTranslation('schedule_passed')}</span>`
      : '';
    ...
    tr.innerHTML = `
      ...
      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">${item.depart_from}</td>
      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">${item.arrive_at}</td>
      <td class="px-6 py-4 whitespace-nowrap text-sm font-bold ${item.isNext ? 'text-primary' : 'text-gray-600 dark:text-gray-400'} ${item.isPassed ? 'line-through' : ''}">${item.depart_time}</td>
      <td class="px-6 py-4 whitespace-nowrap text-sm font-bold ${item.isNext ? 'text-primary' : 'text-gray-600 dark:text-gray-400'} ${item.isPassed ? 'line-through' : ''}">${item.arrive_time}</td>
    `;
    ...
    card.innerHTML = `
      <div class="flex justify-between items-start mb-3 border-b pb-2 dark:border-gray-700 gap-2">
        <div class="flex items-center gap-2 min-w-0">
          ${rankBadge}
          <h4 class="font-bold text-gray-800 dark:text-white truncate">${item.route}</h4>
        </div>
        ...
        ${item.isPassed ? `<span class="text-[10px] text-gray-400 shrink-0">${getTranslation('schedule_passed')}</span>` : ''}
      </div>
      <div class="grid grid-cols-2 gap-4 text-sm">
        <div>
          <p class="text-gray-500 dark:text-gray-400 text-xs" data-i18n="depart_from">${getTranslation('depart_from')}</p>
          <p class="font-medium text-gray-800 dark:text-white">${item.depart_from}</p>
          <p class="font-bold text-primary text-lg ${item.isPassed ? 'line-through opacity-70' : ''}">${item.depart_time}</p>
        </div>
        <div>
          <p class="text-gray-500 dark:text-gray-400 text-xs" data-i18n="arrive_at">${getTranslation('arrive_at')}</p>
          <p class="font-medium text-gray-800 dark:text-white">${item.arrive_at}</p>
