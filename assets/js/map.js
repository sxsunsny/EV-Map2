...
function buildLocationCard(landmarkNameOrArray, onClickSpotId) {
  // รองรับทั้ง string เดียวและ array ของ landmarks
  const names = Array.isArray(landmarkNameOrArray)
    ? landmarkNameOrArray
    : (landmarkNameOrArray ? [landmarkNameOrArray] : []);

  if (names.length === 0) {
    return `<p class="spot-location-card-empty">—</p>`;
  }

  return names.map(landmarkName => {
    ...
    // ใช้รูปจาก tourist_spots.json เท่านั้น
    const imgContent = (matched && matched.image)
      ? `<img src="${matched.image}" alt="${label}" class="spot-location-card-img" loading="lazy"
             onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
           <div class="spot-location-card-placeholder" style="display:none;">
             <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
               <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>
             </svg>
           </div>`
      : `<div class="spot-location-card-placeholder">
           <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
             <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>
           </svg>
         </div>`;
    ...
    return `
      <div class="spot-location-card ${matched ? 'spot-location-card--clickable' : ''}" ${clickAttr}>
...
function openSpotDetail(spot) {
  ...
  // สถานที่ใกล้เคียง: แสดงทุก landmark ใน nearby_landmarks
  nearbyCard.innerHTML = buildLocationCard(spot.nearby_landmarks || []);

  // สถานที่ในอดีต: แสดงเฉพาะ landmark ที่ match กับ spot ใน JSON (มีรูปจาก JSON)
  const matchedNearby = (spot.nearby_landmarks || []).filter(lm => matchLandmarkToSpot(lm));
  pastCard.innerHTML = matchedNearby.length
    ? buildLocationCard(matchedNearby)
    : buildLocationCard(spot.nearby_landmarks ? [spot.nearby_landmarks[0]] : []);
